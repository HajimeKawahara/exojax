...
def test_gpkernel_RBF():
    ...
    assert np.sum(cov) == pytest.approx(5.7267933, 1e-5)
...
def test_gpkernel_RBF_cross():
    ...
    assert np.sum(cov) == pytest.approx(5.801156, 1e-5)
...
def test_average_covariance_gpmodel():
    ...
    assert np.sum(average) == pytest.approx(6.0979223, 1e-5)
    assert np.sum(covariance) == pytest.approx(0.059825122, 1e-5)
...
def test_average_covariance_gpmodel_cross():
    ...
    assert np.sum(average) == pytest.approx(12.213027, 1e-5)
    assert np.sum(covariance) == pytest.approx(1.5004411, 1e-5)
