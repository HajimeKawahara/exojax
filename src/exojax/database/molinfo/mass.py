...
def isotope_molmass(exact_molecule_name):
    ...
    molnumber, isotope_number = exact_molecule_name_to_isotope_number(
        exact_molecule_name
    )
    try:
        simple_molecule_name = exact_molname_exomol_to_simple_molname(
            exact_molecule_name
        )
    ...
    try:
        simple_molecule_name = exact_molname_hitran_to_simple_molname(
            exact_molecule_name
        )
        return molmass_isotope[simple_molecule_name][isotope_number]
    except:
...
def molmass_isotope(simple_molecule_name, db_HIT=True):
    ...
    Example:
        >>> from exojax.database.molinfo  import molmass
        >>> print(molmass("H2"))
        >>> 2.01588
        >>> print(molmass("CO2"))
        >>> 44.0095
    ...
    if simple_molecule_name in molmass_isotope and db_HIT:
        ...
        if db_HIT:
            warn_msg = (
                "db_HIT is set as True, but the molecular name "
                + str(simple_molecule_name)
                + " does not exist in the HITRAN database. So set db_HIT as False."
                + "For reference, all the available molecules in the HITRAN database are as follows"
            )
