...
nus=np.linspace((4000),(4500.0),3000000,dtype=np.float64)
...
xs_dit_lp=dit_xsvector(cnu,indexnu,pmarray,sigmaD,gammaL,Sij,nus,sigmaD_grid,gammaL_grid)
...
xs_dit_lp_f64=dit_xsvector(cnu,indexnu,pmarray,sigmaD,gammaL,Sij,nus,sigmaD_grid,gammaL_grid)
...
plt.plot(wls_dit,xsv_lpf_lp,label="Direct",color="C0",markersize=3,alpha=0.99)
plt.plot(wls_dit,xs_dit_lp,ls="dashed",color="C1",alpha=0.7,label="DIT")
...
plt.plot(wls_dit,np.abs(xs_dit_lp/xsv_lpf_lp-1.)*100,lw=1,alpha=0.5,color="C1",label="MODIT (F32)")
plt.plot(wls_dit,np.abs(xs_dit_lp_f64/xsv_lpf_lp-1.)*100,lw=1,alpha=1,color="C2",label="MODIT (F64)")
plt.yscale("log")
...
plt.savefig("comparison_dit.png", bbox_inches="tight", pad_inches=0.0)
plt.savefig("comparison_dit.pdf", bbox_inches="tight", pad_inches=0.0)
plt.show()
