...
def plottau(nus,dtauM,Tarr=None,Parr=None,unit=None,mode=None,vmin=-3,vmax=3):
    ...
    if unit=="um":
        c=ax.imshow(ltau,vmin=vmin,vmax=vmax,cmap="RdYlBu_r",alpha=0.9,extent=[1.e4/nus[-1],1.e4/nus[0],np.log10(Parr[-1]),np.log10(Parr[0])])
        plt.xlabel("wavelength ($\mu \mathrm{m}$)")
    elif unit=="nm":
        c=ax.imshow(ltau,vmin=vmin,vmax=vmax,cmap="RdYlBu_r",alpha=0.9,extent=[1.e7/nus[-1],1.e7/nus[0],np.log10(Parr[-1]),np.log10(Parr[0])])
        plt.xlabel("wavelength (nm)")
    elif unit=="AA":
        c=ax.imshow(ltau,vmin=vmin,vmax=vmax,cmap="RdYlBu_r",alpha=0.9,extent=[1.e8/nus[-1],1.e8/nus[0],np.log10(Parr[-1]),np.log10(Parr[0])])
        plt.xlabel("wavelength ($\AA$)")
    else:
        c=ax.imshow(np.log10(dtauM),vmin=vmin,vmax=vmax,cmap="RdYlBu_r",alpha=0.9\
