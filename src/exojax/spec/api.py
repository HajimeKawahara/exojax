...
class MdbExomol(CapiMdbExomol):
    ...
    def __init__(self,
        ...
        self.set_broadening(self.jlower, self.jupper)
...
class MdbHit(object):
    ...
    def __init__(self,
        ...
        if numinf is None:
            ...
            if extract:
                if self.path.suffix == '.bz2':
                    tag = str(nurange[0]) + '_' + str(
                        nurange[-1]) + '_' + str(margin)
            ...
            if molnm == '01' or molnm == '02':
                if self.path.name != molnm + '_HITEMP2010':
                    ...
                    print('Warning: Changed the line list path from', path_old,
                          'to', self.path)
            ...
            for k, i in enumerate(range(imin, imax + 1)):
                flname = pathlib.Path(molnm + '_' + numtag[i] +
                                      '_HITEMP2010.par')
        ...
        self.Sij_typ = SijT(self.Ttyp, self.logsij0, self.nu_lines,
                            self.elower, self.QTtyp)
    ...
    def update_value_hapi(self, molec):
        ...
        self.delta_air = np.hstack(
            [self.delta_air,
             hapi.getColumn(molec, 'delta_air')])
        ...
        self._gamma_air = np.hstack(
            [self._gamma_air,
             hapi.getColumn(molec, 'gamma_air')])
        self._gamma_self = np.hstack(
            [self._gamma_self,
             hapi.getColumn(molec, 'gamma_self')])
        ...
        if numtag is not None:
            ...
            try:
                ...
                urllib.request.urlretrieve(url,
                                           str(self.path / numtag / flname))
            except:
                print(url)
                print('HITEMP2010 download failed')
            else:
                print('HITEMP2010 download succeeded')
                shutil.unpack_archive(self.path / numtag / flname,
                                      self.path / numtag)
                ...
                if (self.path / numtag / flname_nonzero).exists():
                    if flname_zero != flname_nonzero:
                        ...
                        print('renamed par file in',
                              self.path / pathlib.Path(numtag), ':',
    ...
    def Qr_HAPI(self, Tarr):
        ...
        for idx, iso in enumerate(self.uniqiso):
            Tmin, Tmax = hapi.get_TMIN_TMAX_FOR_BD_TIPS_2017_PYTHON(
                self.molecid, iso)
            if (max(allT) > Tmax):
                raise ValueError(
                    '%.1f K is outside the supported temperature range of the HITRAN partition function data for isotope #%d of molecule #%d (%.1f -- %.1f K)'
                    % (max(allT), iso, self.molecid, Tmin, Tmax))
            if (min(allT) < Tmin):
                raise ValueError(
                    '%.1f K is outside the supported temperature range of the HITRAN partition function data for isotope #%d of molecule #%d (%.1f -- %.1f K)'
                    % (min(allT), iso, self.molecid, Tmin, Tmax))

            Qrx_iso = jit(vmap(self.QT_iso_interp, (None, 0)))(idx,
                                                               jnp.array(allT))
            Qrx.append(Qrx_iso)
        Qrx = np.array(Qrx)
