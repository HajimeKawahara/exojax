...
@pytest.mark.parametrize("use_x64,tolerance", [(False, 2.0e-7), (True, 4.0e-8)])
def test_trans2E3_matches_scipy(use_x64, tolerance):
    with enable_x64(use_x64):
        dtype = jnp.float64 if use_x64 else jnp.float32
        x = jnp.asarray(np.logspace(-4, 1.9, 1000), dtype=dtype)
        expected = 2.0 * expn(3, np.asarray(x, dtype=np.float64))
        actual = rt.trans2E3(x)
