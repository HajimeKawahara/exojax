...
def read_ExAll(allf, engine="vaex"):
    ...
    dat = dat[
        dat.species.str.startswith("' ").map({False: True, True: False})
    ]  # Remove rows of Reference
    dat = dat[
        dat.species.str.startswith("'_").map({False: True, True: False})
    ]  # Remove rows of Reference
    ...
    dat = dat[
        dat.species.str.slice(start=2, stop=3)
        .str.isupper()
    ...
    dat = dat.drop(
        dat.index[
            np.where(
                np.array(
                    list(
                        map(lambda x: int(x[1].strip("'")), dat.species.str.split(" "))
                    )
                )
                > 3
            )[0]
...
def pickup_param(ExAll):
    ...
    ExAll["nu_lines"] = 1.0e8 / ExAll["wav_lines"]  # [cm-1]<-[AA]
...
def pick_ionE(ielem, iion, df_ionE):
    ...
    def f_droppare(x):
        regex=False
        y = (
            x.str.replace('(', '', regex=regex)
            .str.replace(")", "", regex=regex)
            .str.replace("[", "", regex=regex)
            .str.replace("]", "", regex=regex)
            .str.replace("                                      ", "0", regex=regex)
        )
        return y
