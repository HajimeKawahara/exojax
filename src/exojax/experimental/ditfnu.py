"""Line profile computation using Discrete Integral Transform for free wavenumber
...
def xsvector(nu_lines,sigmaD,gammaL,S,nu_grid,sigmaD_grid,gammaL_grid,dLarray):
    """Cross section vector (DIT/3D version, default) for nu as a free parameter
...
def xsmatrix(nu_lines,sigmaDM,gammaLM,SijM,nu_grid,dgm_sigmaD,dgm_gammaL,dLarray):
    """Cross section matrix for xsvector (DIT/reduced memory version) for nu as a free parameter
