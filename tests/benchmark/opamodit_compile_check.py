"""Manual OpaModit compile-stability check.

This script is for observation only. It does not change ExoJAX behavior or
apply any optimization. It exercises the real ``OpaModit.xsmatrix`` path and
...
- changed-shape calls may either trigger recompilation or fail due to MODIT
  shape rigidity such as cached ``dgm_ngammaL`` expectations
...
class Interpretation:
    ...
    shape_rigidity_or_invalid_shape_observed: str
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Manual compile-stability check for ExoJAX OpaModit.xsmatrix."
    )
    parser.add_argument(
        "--database",
        choices=("exomol", "hitemp"),
        default="exomol",
        help="Mock database backend to use for the real OpaModit path.",
    )
    ...
    parser.add_argument(
        "--dtype-change", choices=("none", "float32", "float64"), default="float64"
    )
    parser.add_argument("--nu-grid-size", type=int, default=20000)
    parser.add_argument("--dit-grid-resolution", type=float, default=0.2)
    parser.add_argument(
        "--alias", choices=("close", "open"), default="close"
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    parser.add_argument(
        "--quiet-json", action="store_true", help="Print only structured JSON output."
    )
    parser.add_argument(
        "--enable-x64",
        action="store_true",
...
def build_log_hint(helper_summary: HelperSignalSummary | None) -> str | None:
    if helper_summary is not None and helper_summary.helper_log_capture_enabled:
        return (
            ...
            "inside repeated same-shape phases, not the cold call or changed-shape probe."
        )
    if os.environ.get("JAX_LOG_COMPILES") == "1":
        return (
            "JAX compile logging is enabled. For same-shape stability, scan for new "
            "compile lines after the first warm call. For changed-shape probes, a compile "
            "or an explicit structural failure may both be informative for MODIT."
        )
    return None
...
def build_interpretation(
    ...
    if result.changed_shape_status == "failed":
        shape_rigidity_or_invalid_shape_observed = "yes"
        changed_shape_category = "Structural / invalid-shape behavior"
    else:
        shape_rigidity_or_invalid_shape_observed = "no"
        if changed_shape_recompilation_likely in ("yes", "no"):
            changed_shape_category = "Expected / normal"
        else:
            changed_shape_category = "Not enough evidence"

    if result.dtype_changed_status == "failed":
        dtype_change_category = "Structural / unsupported behavior"
    ...
    if same_shape_top_level_timing_appears_stable == "yes":
        conclusion.append(
            "Top-level repeated same-shape timings remain close to the warm call, which suggests stable top-level caching for the tested MODIT shape."
        )
        ...
        conclusion.append(
            "Top-level same-shape timing stability is unclear from timing alone."
        )
    ...
    if result.changed_shape_status == "failed":
        conclusion.append(
            "The changed-shape probe failed on the existing OpaModit instance, which suggests structural shape rigidity or invalid-shape behavior rather than a clean recompilation measurement."
        )
        ...
        conclusion.append(
            "Changed-shape behavior is unclear from timing alone."
        )
    ...
    if result.dtype_changed_status == "skipped":
        ...
        conclusion.append(
            "The dtype-change probe failed structurally, so dtype-driven recompilation could not be measured cleanly."
        )
    ...
    return Interpretation(
        ...
        shape_rigidity_or_invalid_shape_observed=shape_rigidity_or_invalid_shape_observed,
...
def print_human_summary(
    ...
    print("opamodit_compile_check")
    print("run_config")
    print(f"database: {result.database}")
    print(f"alias: {result.alias}")
    print(f"dit_grid_resolution: {result.dit_grid_resolution}")
    ...
    print(f"shape_rigidity_or_invalid_shape_observed: {interpretation.shape_rigidity_or_invalid_shape_observed}")
...
def execute_measurement(args: argparse.Namespace) -> tuple[TimingResult, DerivedMetrics]:
    ...
    opa = OpaModit(
        ...
        Tarr_list=Tarr_base,
        Parr=Parr_base,
        dit_grid_resolution=args.dit_grid_resolution,
        allow_32bit=True,
        alias=args.alias,
    )
