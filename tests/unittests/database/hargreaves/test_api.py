...
def test_line_strength_hargreaves():
    ...
    df = mdb.df
    Sij0 = df["Sij0"].values
    assert np.all(np.isfinite(Sij0))
    # Independently rescale the bundled CSV from 2200 K to 296 K using
    # scale=1/3, Q(2200)=11691.1386, Q(296)=215.3488, and the project hc/k.
    assert np.sum(Sij0) == pytest.approx(1.3015008918908924e-17, rel=1.0e-6, abs=0.0)
