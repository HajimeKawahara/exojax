...
def test_generate_closed_lpffilter():
    filter_length, nsigmaD, ngammaL = _test_values_lpffilter()
    shapefilter = generate_closed_lpffilter(filter_length, nsigmaD, ngammaL)
...
def test_generate_open_lpffilter():
    filter_length, nsigmaD, ngammaL = _test_values_lpffilter()
    shapefilter = generate_open_lpffilter(filter_length, nsigmaD, ngammaL)
...
def _refvalue_closed_lpffilter():
    ...
    filter_length, nsigmaD, ngammaL = _test_values_lpffilter()
    closeref = jnp.concatenate([openref[filter_length:], openref[:filter_length]])
    return closeref
...
def _test_values_lpffilter():
    ...
    nsigmaD = 1.0
    ngammaL = 1.0
    return filter_length, nsigmaD, ngammaL
...
def lpffilter_agreement_with_fold_voigt_kernel_logst(figure=False):
    ...
    filter_length = len(nu_grid)
    vk = fold_voigt_kernel_logst(
        jnp.fft.rfftfreq(2 * filter_length, 1),
        jnp.log(nsigmaD),
        log_ngammaL_grid,
        filter_length,
        pmarray,
    ...
    lpffilter = vmap_generate_lpffilter(filter_length, nsigmaD, ngammaL_grid)
