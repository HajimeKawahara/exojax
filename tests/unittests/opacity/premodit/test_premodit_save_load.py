...
def _build_minimal_ready_opa() -> OpaPremodit:
    ...
    opa.set_aliasing()
    opa._sets_capable_opacalculators()
...
def test_save_and_load_roundtrip(tmp_path, format):
    ...
    actual_cross_section = loaded.xsvector(500.0, 1.0)
    assert np.all(np.isfinite(expected_cross_section))
    assert np.all(expected_cross_section > 0.0)
    np.testing.assert_allclose(
        actual_cross_section, expected_cross_section, rtol=1.0e-12, atol=0.0
    )
