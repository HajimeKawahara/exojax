...
if False:
    nus = numpy.logspace(numpy.log10(1900.0), numpy.log10(
        2300.0), 160000, dtype=numpy.float64)
...
xsmode = 'MODIT'
nus, wav, res = wavenumber_grid(1900.0, 2300.0, 320000, 'cm-1', xsmode=xsmode)
...
autort = AutoRT(nus, 1.e5, 2.33, Tarr, Parr, xsmode=xsmode,
                autogridconv=False)  # g=1.e5 cm/s2, mmw=2.33
...
xsmode = 'DIT'
nus_, wav, res = wavenumber_grid(1900.0, 2300.0, 320000, 'cm-1', xsmode=xsmode)
Parr = numpy.logspace(-8, 2, 100)  # 100 layers from 10^-8 bar to 10^2 bar
Tarr = 500.*(Parr/Parr[-1])**0.02
