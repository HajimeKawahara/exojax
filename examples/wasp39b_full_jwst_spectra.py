...
# NIRISS ORDER 1
wav_obs_niriss_1, rp_mean_niriss_1, rp_err_low_niriss_1, rp_err_high_niriss_1 = np.loadtxt(
    "wasp39_data/niriss_order1.txt", unpack=True
)

# NIRISS ORDER 2
wav_obs_niriss_2, rp_mean_niriss_2, rp_err_low_niriss_2, rp_err_high_niriss_2 = np.loadtxt(
    "wasp39_data/niriss_order2.txt", unpack=True
)

# NIRSPEC
wav_obs_nirspec = np.load(
    "wasp39_data/wavelength.npy"
)  # observed wavelength grid (nm)
rp_mean_nirspec = np.load(
    "wasp39_data/wasp39b_nirspec_g395h_rp_mean.npy"
)  # mean R_p/R_s spectrum
rp_std_nirspec = np.load(
    "wasp39_data/wasp39b_nirspec_g395h_rp_std.npy"
)  # 1 std uncertainty

# MIRI 
import h5py
infile = "wasp39_data/miri.h5"
with h5py.File(infile, "r") as f:
        dppm = np.array(f["dppm"])                     # 
        dppm_err = np.array(f["dppm_error"])           # 
        wl = np.array(f["wavelength"])

ppmtor = 1e-6  # conversion factor from ppm to Rp/Rs
microntonm = 1e3  # conversion factor from micron to nm

wav_obs_miri = wl*microntonm
rp_mean_miri = np.sqrt(dppm*ppmtor)
rp_err_miri = dppm_err*ppmtor / (2 * np.sqrt(dppm*ppmtor)) 


# Convert from wavelength to wavenumber for modelling
inst_niriss_nus_1 = wav2nu(wav_obs_niriss_1, "nm")
inst_niriss_nus_2 = wav2nu(wav_obs_niriss_2, "nm")
...
_svi_params, losses, init_strategy, svi_median, svi_guide = run_svi(
    rng_key_, rp_mean=rp_mean_nirspec, rp_std=rp_std_nirspec
)
...
mcmc.run(rng_key_, rp_mean=rp_mean_nirspec, rp_std=rp_std_nirspec)
...
predictions = pred(rng_key_, rp_mean=None, rp_std=rp_std_nirspec)
...
svi_mu = svi_pred(rng_plot, rp_mean=rp_mean_nirspec, rp_std=rp_std_nirspec)["rp_mu"][0]
...
svi_samples = svi_guide[-1].sample_posterior(
    rng_svi,
    _svi_params,
    rp_mean=rp_mean_nirspec,
    rp_std=rp_std_nirspec,
    sample_shape=(1000,),
)
...
plot_overlay(wav_obs_nirspec, rp_mean_nirspec, rp_std_nirspec, predictions["rp"], svi_mu, overlay_plot_path)
