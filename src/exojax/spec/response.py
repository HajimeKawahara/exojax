...
def rotkernel_jvp(primals, tangents):
    ...
    dHdx = jnp.where(x2 <= 1.0, - jnp.pi*x*u1 +
                     2.0/3.0*x/jnp.sqrt(1.0-x2)*(-3.0+3.0*u1+u2+2.0*u2*x2), 0.0)
    dHdu1 = jnp.where(x2 <= 1.0, -2.0*jnp.sqrt(1.0-x2) +
                      jnp.pi/2.0*(1.0-x2), 0.0)
    dHdu2 = jnp.where(x2 <= 1.0, -3.0*(1.0+2.0*x2)*(2.0*jnp.sqrt(1.0-x2)), 0.0)
