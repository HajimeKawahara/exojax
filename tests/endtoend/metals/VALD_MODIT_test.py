...
def test_VALD_MODIT():
    ...
    beta_inst = resolution_to_gaussian_std(
        Rinst
    )  # equivalent to beta=c/(2.0*np.sqrt(2.0*np.log(2.0))*R)
    ...
    mdbH2O = api.MdbExomol(
        ".database/H2O/1H2-16O/POKAZATEL", nus, crit=1e-50, gpu_transfer=True
    )  # ,crit = 1e-40)
    mdbTiO = api.MdbExomol(
        ".database/TiO/48Ti-16O/Toto", nus, crit=1e-50, gpu_transfer=True
    )  # ,crit = 1e-50)
    ...
    dgm_ngammaL_VALD = setdgm_vald_all(
        asdb, PH_ref, PHe_ref, PHH_ref, R, fT, res, T0_test, alpha_test
    )
    dgm_ngammaL_FeH = setdgm_exomol(
        mdbFeH, fT, Parr, R, molmassFeH, res, T0_test, alpha_test
    )
    dgm_ngammaL_H2O = setdgm_exomol(
        mdbH2O, fT, Parr, R, molmassH2O, res, T0_test, alpha_test
    )
    dgm_ngammaL_OH = setdgm_exomol(
        mdbOH, fT, Parr, R, molmassOH, res, T0_test, alpha_test
    )
    dgm_ngammaL_TiO = setdgm_exomol(
        mdbTiO, fT, Parr, R, molmassTiO, res, T0_test, alpha_test
    )
    ...
    # Hminus
    dtau_Hm = dtauHminus_mmwl(
        nus, Tarr, Parr, dParr, VMR_e * ONEARR, VMR_H * ONEARR, mmw, ga
    )

    # CIA
    dtauc_H2H2 = dtauCIA_mmwl(
        nus,
        Tarr,
        Parr,
        dParr,
    ...
    # Summations
    dtau = (
        dtauatom + dtaum_FeH + dtaum_H2O + dtaum_OH + dtaum_TiO + dtau_Hm + dtauc_H2H2
    )
