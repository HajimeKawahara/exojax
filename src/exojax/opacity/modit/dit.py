...
@jit
def xsvector(
    cnu, indexnu, pmarray, sigmaD, gammaL, S, nu_grid, sigmaD_grid, gammaL_grid
):
...
@jit
def xsmatrix(
    cnu, indexnu, pmarray, sigmaDM, gammaLM, SijM, nu_grid, dgm_sigmaD, dgm_gammaL
):
...
def sigma_voigt(dgm_sigmaD, dgm_gammaL):
    ...
    Returns:
        sigma
    """
    fac = 2.0 * np.sqrt(2.0 * np.log(2.0))
    fdgm_gammaL = jnp.min(dgm_gammaL, axis=1) * 2.0
    fdgm_sigmaD = jnp.min(dgm_sigmaD, axis=1) * fac
    fv = jnp.min(
        0.5346 * fdgm_gammaL + jnp.sqrt(0.2166 * fdgm_gammaL**2 + fdgm_sigmaD**2)
    )
    sigma = fv / fac
    return sigma
...
def vald(adb, Tarr, PH, PHe, PHH):
    ...
    """
    from exojax.opacity.lpf.lpf import vald as vald_
...
def dtauM_vald(
    ...
    """
    zero_to_ones = lambda arr: jnp.where(arr != 0, arr, 1.0)
    ...
    def floop(xi, null):
        ...
        indexnu_p = jnp.array(
            padding_2Darray_for_each_atom(indexnu[None, :], adb, sp).reshape(
                indexnu.shape
            ),
            dtype="int32",
        )
        ...
        MMRmetalMod = mods_uspecies_list[
            i
        ]  # add_to_deal_with_individual_elemental_abundance
        MMR_X_I = (
