...
from exojax.spec import contdb, molinfo
...
def test_plg_methane():
    ...
    nus, wav, reso = wavenumber_grid(wls,
                                     ...
                                     unit="AA",
                                     xsmode="modit")
    mdb_orig = api.MdbExomol('.database/CH4/12C-1H4/YT10to10/', nus, \
    ...
    nusc, wavc, resoc = wavenumber_grid((wll + wls - assess_width) / 2,
                                        (wll + wls + assess_width) / 2,
                                        20,
                                        unit="AA",
                                        xsmode="modit")
    mdbc = api.MdbExomol('.database/CH4/12C-1H4/YT10to10/',
                         nusc,
                         gpu_transfer=True)

    ts = time.time()
    coefTgue = plg.optimize_coefTgue(Tgue, nusc, mdbc, molmassCH4, Nelower,
                                     errTgue)
    te = time.time()
    ...
    Nnugrid = len(nus)
    mdb, cnu, indexnu = plg.gather_lines(mdb, Ngamma, Nnugrid, Nelower, nus,
                                         cnu, indexnu, qlogsij0, qcnu,
                                         elower_grid, alpha_ref_grid,
                                         n_Texp_grid, frozen_mask,
                                         nonzeropl_mask)
    ...
    res = 0.2
    dgm_ngammaL = set_ditgrid_matrix_exomol(mdb, fT, Parr, R, molmassCH4, res,
                                            T0_test, alpha_test)
    dgm_ngammaL_orig = set_ditgrid_matrix_exomol(mdb_orig, fT, Parr, R,
                                                 molmassCH4, res, T0_test,
                                                 alpha_test)

    #a core driver
    def frun(Tarr, MMR_, Mp, Rp, u1, u2, RV, vsini):
        g = 2478.57730044555 * Mp / Rp**2
        SijM_, ngammaLM_, nsigmaDl_ = modit.exomol(mdb, Tarr, Parr, R,
                                                   molmassCH4)
        xsm_ = modit.xsmatrix(cnu, indexnu, R, pmarray, nsigmaDl_, ngammaLM_,
                              SijM_, nus, dgm_ngammaL)
    ...
    def frun_orig(Tarr, MMR_, Mp, Rp, u1, u2, RV, vsini):
        g = 2478.57730044555 * Mp / Rp**2
        SijM_, ngammaLM_, nsigmaDl_ = modit.exomol(mdb_orig, Tarr, Parr, R,
                                                   molmassCH4)
        xsm_ = modit.xsmatrix(cnu_orig, indexnu_orig, R, pmarray, nsigmaDl_,
                              ngammaLM_, SijM_, nus, dgm_ngammaL_orig)
        #abs is used to remove negative values in xsv
        dtaum = dtauM(dParr, jnp.abs(xsm_), MMR_ * ONEARR, molmassCH4, g)
        #CIA
        dtaucH2H2 = dtauCIA(nus, Tarr, Parr, dParr, vmrH2, vmrH2, mmw, g,
