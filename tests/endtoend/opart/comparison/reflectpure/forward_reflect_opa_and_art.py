...
art = ArtReflectPure(
    nu_grid=nu_grid, pressure_top=1.0e-5, pressure_btm=1.0e1, nlayer=200
)
...
#with jax.profiler.trace("/tmp/jax-trace", create_perfetto_link=True):
    for i in tqdm.tqdm(range(Ntry)):
        ...
        mixing_ratio = art.constant_mmr_profile(0.0003)
        ...
        flux = art.run(dtau, single_scattering_albedo, asymmetric_parameter, reflectivity_surface, incoming_flux)
        fluxarr.append(flux)
