...
molmassM = molinfo.molmass_major_isotope('H2O')  # molecular mass (H2O)
...
for j, i in enumerate([259, 236, 56]):
    ...
    plt.plot(Tarr, SijM[:, i]*fac, color='C' +
             str(j), ls=lsa[j], alpha=1.0, label=w)
    plt.text(Tmax[i], SijM[imax[i], i]*fac,
