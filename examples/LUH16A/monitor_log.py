...
in your shell $ nohup python -u FidEMu/fit.py >& FidEMu/log & $ python
monitor_log.py FidEMu/log
"""
...
#        terminalplot.plot(list(np.log10(step)),list(np.log10(ctime)))
    ...
    print('median/chain:', np.median(step),
          '|min-max', np.min(step), '-', np.max(step))
    print('total time=', round(np.sum(ctime)/3600, 3),
          'h / ', round(np.sum(ctime)/60, 1), 'min')
