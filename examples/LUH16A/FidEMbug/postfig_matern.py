...
molmassCO=molinfo.molmass_major_isotope("CO") #molecular mass (CO)
molmassH2O=molinfo.molmass_major_isotope("H2O") #molecular mass (H2O)

#LOADING CIA
mmrH2=0.74
mmrHe=0.25
molmassH2=molinfo.molmass_major_isotope("H2")
molmassHe=molinfo.molmass_major_isotope("He")
...
def predmod(nu1,y1,e1,pos,i):    
    ...
    def Matern(t,td,tau,a):
        Dt = t - jnp.array([td]).T
        fac=jnp.sqrt(3.0)*jnp.abs(Dt)/tau
        K=a*(1.0+fac)*jnp.exp(-fac)
        return K
    ...
    cov = a*Matern(t,t,tau,a) + jnp.diag(errall**2)
    covx= a*Matern(t,td,tau,a)
    covxx = a*Matern(td,td,tau,a) + jnp.diag(errall**2)
    IKw=cov 
