...
def set_C_to_O(fastchem, c_to_o):    
    """Setting up C/O ratio by scaling the O abundance as a function of the C/O ratio (Molliére et al. 2015)
    
    Args:
        fastchem: fastchem object created via pyfastchem.FastChem( str(dir_fastchem)+"input/element_abundances_solar_ext.dat",
...
def set_M_to_H(fastchem, M_to_H):
    """Setting up [M/H] by scaling all of the metallic abundances but H and He by 10**M_to_H 
...
def continuum_vmr_fc2(fastchem,mixing_ratios):
    """Extracting the volume mixing ratio (VMR) of spectral continuum-related chemical elements
