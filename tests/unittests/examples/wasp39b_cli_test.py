...
def test_check_inputs_json_does_not_require_ckd_tables_for_premodit(tmp_path):
    ...
    assert snapshot_status["status"] == "ok"
...
def test_ckd_wide_hmc_smoke_with_synthetic_table_writes_outputs(tmp_path):
    ckd_root = tmp_path / "ckd"
    ...
    assert "Matplotlib created a temporary cache directory" not in result.stderr
    assert "Both input wavelength and output wavenumber are in ascending order" not in result.stderr
    assert "Posterior samples saved" in result.stdout
    ...
    assert run_config["data_mode"] == "wide"
    ...
    assert run_config["chain_method"] == "parallel"
    assert run_config["max_tree_depth"] == 2
    assert run_config["ckd_sources"]["H2O"].endswith("synthetic_h2o.h5")
    table_summary = run_config["ckd_table_summary"]["H2O"]
    assert table_summary["molmass"] == 18.0
    assert table_summary["n_temperature"] == 2
    assert table_summary["n_pressure"] == 2
    assert table_summary["n_g"] == 3
    assert table_summary["weight_sum"] == 1.0
    assert run_config["channels"] == [
        "niriss_order1",
        "niriss_order2",
        "nirspec_g395h",
        "miri_lrs",
    ]
    assert run_config["cia_pairs"] == []
    assert run_config["n_observed"] == 8
    assert run_config["ckd_band_summary"]["n_bands"] > 1
    assert run_config["ckd_band_summary"]["n_g"] == 3
    retrieval_summary = run_config["retrieval_channel_summary"]
    assert set(retrieval_summary) == set(run_config["channels"])
    assert sum(item["n_selected"] for item in retrieval_summary.values()) == 8
    assert sum(item["n_selected"] > 0 for item in retrieval_summary.values()) > 1

    with np.load(output_dir / "observed_data.npz") as data:
        assert data["wavelength_nm"].shape == (8,)
        assert data["radius_ratio"].shape == (8,)
        assert data["channel_index"].shape == (8,)
        assert np.unique(data["channel_index"]).size > 1
    ...
    assert run_status["data_mode"] == "wide"
    assert run_status["opacity_mode"] == "ckd"
    assert run_status["cia_pairs"] == []
    assert run_status["ckd_sources"]["H2O"].endswith("synthetic_h2o.h5")
    assert run_status["input_status"]["ready_for_local_run"] is True
    assert run_status["input_status"]["problems"] == []
    assert (
        run_status["input_status"]["ckd_sources"]["H2O"]["resolved_table"]
        == run_status["ckd_sources"]["H2O"]
    )
    assert (
        run_status["input_status"]["ckd_sources"]["H2O"][
            "resolved_table_metadata"
        ]["size_bytes"]
        > 0
    )
    assert run_status["ckd_band_summary"]["n_g"] == 3
    assert run_status["ckd_table_summary"]["H2O"]["molmass"] == 18.0
    ...
    assert set(run_status["retrieval_channel_summary"]) == set(run_config["channels"])
    assert (
        sum(
            item["n_selected"]
            for item in run_status["retrieval_channel_summary"].values()
        )
        == 8
    )
    ...
    assert run_status["chain_method"] == "parallel"
    assert run_status["max_tree_depth"] == 2
    assert run_status["jax_default_backend"] == "cpu"
    assert run_status["jax_devices"]
    assert run_status["posterior_shapes"]["logVMR_H2O"] == [1]
    assert run_status["predictive_shapes"]["rp_mu"] == [1, 8]
    assert run_status["posterior_finite"]["logVMR_H2O"] is True
    assert run_status["predictive_finite"]["rp_mu"] is True
    ...
    assert run_status["skipped_artifacts"] == []
    assert run_status["all_expected_artifacts_present"] is True
    assert run_status["shape_checks"] == {
        "posterior_rp_mu_matches_observed": True,
        "predictive_rp_matches_observed": True,
        "predictive_rp_mu_matches_observed": True,
    }
    assert run_status["ready_for_inspection"] is True


def test_ckd_hmc_smoke_can_skip_diagnostic_plots(tmp_path):
    ckd_root = tmp_path / "ckd"
    output_dir = tmp_path / "output"
    write_synthetic_exomolop_ckd(
        ckd_root,
    ...
    with open(output_dir / "run_config.json") as handle:
        run_config = json.load(handle)
    assert run_config["skip_diagnostic_plots"] is True
