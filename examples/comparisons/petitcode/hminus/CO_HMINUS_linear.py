...
print(len(pressures))
with open('../co_exojax/Gl229/pressures.dat', 'w') as f:                                
...
temperature=np.linspace(2000.0,3000.0,len(temperature)) #linear
#temperature=np.ones(len(temperature))*1000.0 #linear


abundances = {}
abundances['H2'] = 0.75 * np.ones_like(temperature)
abundances['He'] = 0.24 * np.ones_like(temperature)
abundances['H'] = 0.0001 * np.ones_like(temperature)
abundances['H-'] = 0.0001 * np.ones_like(temperature)
abundances['e-'] = 0.0001 * np.ones_like(temperature)
...
with open('CO_Hminus_Linear.dat', 'w') as f:
...
plt.show()
plt.savefig('emission-HR.pdf',bbox_inches='tight')
plt.clf()
...
plt.ylabel(r'Planet flux $F_\nu$ (10$^{-6}$ erg cm$^{-2}$ s$^{-1}$ Hz$^{-1}$)')
plt.show()
plt.savefig('emission-HR-zoom.pdf',bbox_inches='tight')
plt.clf()
