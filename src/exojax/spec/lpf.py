...
def exomol(mdb, Tarr, Parr, molmass):
    ...
    SijM = jit(vmap(line_strength, (0, None, None, None, 0, None)))(
        Tarr, mdb.logsij0, mdb.dev_nu_lines, mdb.elower, qt, mdb.Tref
    )
...
def vald_each(
    ...
    """Computes VALD line information required for LPF for separated each species
...
def ljert(x, a):
    ...
    """
    r2 = x * x + a * a
    return jnp.where(r2 < 111.0, imwofz(x, a), jnp.imag(asymptotic_wofz(x, a)))
...
def hjert(x, a):
    ...
    """
    r2 = x * x + a * a
    return jnp.where(r2 < 111.0, rewofz(x, a), jnp.real(asymptotic_wofz(x, a)))
